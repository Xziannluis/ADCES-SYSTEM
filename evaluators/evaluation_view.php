...
<head>
    ...
    <style>
        ...
        /* Print clarity: disabled inputs should not look "greyed out" */
        @media print {
...
<div class="main-content">
    <div class="container-fluid">
        ...
        <script>
            // Browser-native PDF: open print dialog (user can choose "Save as PDF")
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="evaluation-section">
                    ...
                    <!-- AI generation (read-only preview) -->
                    ...
                    <script>
                        (function () {
                            ...
                            const debugText = document.getElementById('aiDebugText');
                            ...
                            async function generateAI() {
                                ...
                                if (!strengthsEl || !improvementEl || !recEl) {
                                    alert('AI fields are missing on this page.');
                                    return;
                                }

                                // Build the same shape the AI service expects
                                const payload = {
                                    faculty_name: <?php echo json_encode($eval['teacher_name'] ?? ''); ?>,
                                    department: <?php echo json_encode($eval['teacher_department'] ?? ($eval['department'] ?? '')); ?>,
                                    subject_observed: <?php echo json_encode($eval['subject_observed'] ?? ''); ?>,
                                    observation_type: <?php echo json_encode($eval['observation_type'] ?? ''); ?>,
                                    averages: {
                                        communications: <?php echo json_encode($communicationsAvg); ?>,
                                        management: <?php echo json_encode($managementAvg); ?>,
                                        assessment: <?php echo json_encode($assessmentAvg); ?>,
                                        overall: <?php echo json_encode($overallAvg); ?>
                                    },
                                    // Passing ratings as empty is OK; app.py will fall back to template-based feedback.
                                    ratings: {}
                                };

                                if (btn) btn.disabled = true;
                                setStatus('Generating… (first time can take a while)', true);
                                ...
                                try {
                                    ...
                                    if (!res.ok || !data || data.success !== true) {
                                        ...
                                        const msg = (data && data.message) ? data.message : 'AI generation failed.';
                                        setStatus(msg, true);
                                        alert(msg);
                                        return;
                                    }
                                    ...
                                    const out = data.data || {};
                                    if (out.strengths) strengthsEl.value = out.strengths;
                                    if (out.improvement_areas) improvementEl.value = out.improvement_areas;
                                    if (out.recommendations) recEl.value = out.recommendations;

                                    setStatus('Done', true);
                                } catch (err) {
                                    console.error(err);
                                    setStatus('Error: ' + (err && err.message ? err.message : String(err)), true);
                                    alert('AI generation error. Check if the Python AI service is running.');
                                } finally {
                        ...
                        <div class="col-md-6">
                            <div class="border p-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" value="<?php echo h($eval['rater_signature'] ?? ''); ?>" disabled>
                                </div>
                                <div class="mb-3">
                        ...
                        <div class="col-md-6">
                            <div class="border p-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" value="<?php echo h($eval['faculty_signature'] ?? ''); ?>" disabled>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Date</label>
