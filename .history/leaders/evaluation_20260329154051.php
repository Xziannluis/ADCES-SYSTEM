<?php
// Redirect to evaluators/evaluation.php (consolidated)
...
header("Location: ../evaluators/evaluation.php" . $qs);
exit();
            ...
            <div class="card mb-4" id="teacherSelection">
                ...
                <div class="card-body">
                    ...
                    <div class="list-group" id="teacherList">
                        ...
                        ?>
                        <?php
                            // Observation plan acknowledgment check
                            ...
                            // Form type and ISO completion tracking
                            $teacher_form_type = $teacher_row['evaluation_form_type'] ?? 'iso';
                            $iso_done = isset($completedIsoTeachers[(int)$teacher_row['id']]);
                        ?>
                        <div class="list-group-item teacher-item <?php echo $can_evaluate_now ? '' : 'disabled'; ?>" data-teacher-id="<?php echo $teacher_row['id']; ?>" data-has-schedule="<?php echo $has_schedule ? '1' : '0'; ?>" data-can-evaluate-now="<?php echo $can_evaluate_now ? '1' : '0'; ?>" data-schedule-message="<?php echo htmlspecialchars($schedule_message, ENT_QUOTES); ?>" data-block-reason="<?php echo htmlspecialchars($schedule_block_message, ENT_QUOTES); ?>" data-subject="<?php echo htmlspecialchars($teacher_row['evaluation_subject'] ?? '', ENT_QUOTES); ?>" data-subject-area="<?php echo htmlspecialchars($teacher_row['evaluation_subject_area'] ?? '', ENT_QUOTES); ?>" data-room="<?php echo htmlspecialchars($teacher_row['evaluation_room'] ?? '', ENT_QUOTES); ?>" data-form-type="<?php echo htmlspecialchars($teacher_form_type, ENT_QUOTES); ?>" data-iso-done="<?php echo $iso_done ? '1' : '0'; ?>">
        ...
        function initializeTeacherSelection() {
            ...
            document.querySelectorAll('.teacher-item').forEach(item => {
                item.addEventListener('click', function(e) {
                    ...
                    if (formType === 'peac') {
                        window.location.href = '../evaluators/evaluation_peac.php?teacher_id=' + encodeURIComponent(teacherId);
                        return;
                    }
