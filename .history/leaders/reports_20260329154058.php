<?php
// Redirect to evaluators/reports.php (consolidated)
...
header("Location: ../evaluators/reports.php" . $qs);
exit();
...
// Presidents/VPs only see their own evaluations (president + vice_president)
// Get all president/VP user IDs
$leaderIds = [];
try {
    $leaderStmt = $db->prepare("SELECT id FROM users WHERE role IN ('president', 'vice_president')");
    $leaderStmt->execute();
    $leaderIds = $leaderStmt->fetchAll(PDO::FETCH_COLUMN);
} catch (Exception $e) {
    $leaderIds = [];
}
