    ...
    if(isset($_POST['action'])) {
        switch($_POST['action']) {
            case 'create':
                ...
                // No special mapping needed - values are stored as-is
                ...
                // If creating a teacher or evaluator account, also create/update the teacher record
                // so they appear in teacher lists and can be evaluated (if they also teach)
                if (in_array($role, ['teacher', 'dean', 'principal', 'chairperson', 'subject_coordinator', 'grade_level_coordinator']) && $createResult === true) {
...
<?php endif; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const secondaryDeptItems = document.querySelectorAll('.secondary-dept-item');

            function syncSecondaryDepts(mainDepartment) {
                secondaryDeptItems.forEach(item => {
                    const deptVal = item.getAttribute('data-dept-value');
                    const cb = item.querySelector('.secondary-department-checkbox');
                    if (deptVal === mainDepartment) {
            ...
            if (editTeacherDepartment) {
                editTeacherDepartment.addEventListener('change', function() {
                    syncSecondaryDepts(editTeacherDepartment.value);
                });
            }
            ...
            editButtons.forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    syncSecondaryDepts(department);
                    editModal.show();
                });
            });

            syncSecondaryDepts(editTeacherDepartment ? editTeacherDepartment.value : '');
        });

        // Confirmation dialog for deactivate/activate actions
        document.querySelectorAll('form.d-inline').forEach(function(form) {
            form.addEventListener('submit', function(e) {
                var action = form.querySelector('input[name="action"]');
                if (action && action.value === 'deactivate') {
                    var userName = form.closest('tr').querySelector('td:nth-child(2)').textContent.trim();
