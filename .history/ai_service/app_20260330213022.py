...
def _template_contradicts_ratings(
    ...
    if max_scale <= 4.0:
        ...
        # ISO: Be stricter for improvements (reject when indicator is 4+),
        # but more lenient for recommendations (only reject when 5, since
        # recommending growth on a 4/5 indicator is still reasonable).
        high_threshold = 5.0 if field_name == "recommendations" else 4.0
        low_threshold = 2.5
...
_OPENER_POOL_IMPROVEMENT = [
    ...
    "An area that warrants further professional development is",
    "One aspect that requires continued attention is",
    "Based on the lesson evidence,",
    "A dimension of instruction requiring focused effort is",
    "An opportunity for professional growth was noted in how",
    "Further refinement is needed in",
    "The lesson indicated a need for development in",
    "An area with clear potential for improvement is",
    "Classroom evidence suggests a need to strengthen",
    "A developing area in the instructional practice is",
    "An aspect of instruction that merits focused attention is",
    "There is scope for strengthening",
    "The lesson data indicates that growth is needed in",
    "An area where more deliberate practice would be beneficial involves",
    "Room for measured improvement was noted in",
    "The evidence suggests a need for more consistent practice in",
    "An emerging area for instructional development is",
    "The lesson reflected a need for more intentional focus on",
    "Instructional evidence points to a growth area in",
]
...
def _make_three_options(base_text: str, req: GenerateRequest, field_name: str, retrieved: List[Dict[str, Any]]) -> List[str]:
    ...
    for item in retrieved:
        ...
        if not text or len(text.split()) < 5:
            continue
        # Trim to 3 sentences max then strip generic filler
