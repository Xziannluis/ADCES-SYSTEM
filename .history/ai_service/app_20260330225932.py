...
def _build_rating_context_sentence(
    ...
    max_scale = 4.0 if _is_peac_request(req) else 5.0
    rating_label = _score_band(rating, max_scale).lower()
    ...
    is_top = rating >= max_scale  # perfect score
    is_high = rating >= (max_scale - 1)  # 4+ for ISO, 3+ for PEAC
    is_mid = rating >= (max_scale / 2)
    is_low = rating < (max_scale / 2)

    if field_name == "strengths":
        if is_top:
            return (
                f"The evaluator's assessment confirmed that this is one of the teacher's most well-developed "
                f"instructional competencies, reflecting consistent and exemplary practice in {canonical_domain.lower()}."
            )
        if is_high:
            return (
                f"This was identified as a reliable area of competence in the evaluator's assessment, "
                f"contributing positively to the teacher's overall {overall_label} instructional performance."
            )
        return (
            f"The evaluator recognized this as a developing strength that, with sustained practice, "
            f"can further enhance the quality of instruction within {canonical_domain.lower()}."
        )
    elif field_name == "areas_for_improvement":
        if is_low:
            return (
                f"The evaluator's assessment highlighted this as the most critical area requiring focused professional "
                f"development, as it significantly impacts the overall quality of {canonical_domain.lower()}."
            )
        if is_mid:
            return (
                f"Based on the evaluator's assessment, this area shows room for meaningful growth and would benefit "
                f"from deliberate, sustained attention to strengthen the teacher's practice in {canonical_domain.lower()}."
            )
        return (
            f"While the evaluator noted competence in this area, there remains an opportunity "
            f"for refinement that could elevate the overall instructional quality."
        )
    else:  # recommendations
        if is_low:
            return (
                f"The evaluator's findings indicate that prioritizing this recommendation would directly address "
                f"one of the most significant gaps identified in {canonical_domain.lower()} and support measurable instructional improvement."
            )
        if is_mid:
            return (
                f"Implementing this recommendation would respond to the evaluator's assessment and contribute "
                f"to strengthening the teacher's overall instructional effectiveness in {canonical_domain.lower()}."
            )
        return (
            f"This recommendation aligns with the evaluator's overall assessment and, if consistently applied, "
            f"would further elevate the quality of instruction."
        )
...
def _build_domain_rating_summary(
    ...
    if field_name == "strengths":
        ...
        if strongest:
            return (
                f"The evaluator's assessment identified {strongest.lower()} as the teacher's "
                f"most well-developed domain, reflecting consistent and effective instructional practice "
                f"that positively contributes to the overall {overall_label} teaching performance."
            )
        ...
        if weakest:
            if field_name == "areas_for_improvement":
                return (
                    f"The evaluator's assessment indicated that {weakest.lower()} is the domain "
                    f"most in need of focused professional development, representing a key opportunity "
                    f"to strengthen the teacher's overall instructional effectiveness."
                )
            else:
                return (
                    f"Based on the evaluator's assessment, sustained improvement efforts in "
                    f"{weakest.lower()} would yield the most significant gains in the teacher's "
                    f"overall instructional quality and professional growth."
                )
    return ""
