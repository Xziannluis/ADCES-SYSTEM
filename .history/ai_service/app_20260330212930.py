...
def _ensure_critical_indicator_mentioned(
    ...
    # Pick an opener that doesn't clash with existing options
    _STRENGTH_OPENERS = [
        "A clear strength in the lesson was",
        "A distinct strength observed was",
        "A noteworthy strength in the lesson was",
    ]
    _IMPROVEMENT_OPENERS = [
        "An area that would benefit from focused attention is",
        "One aspect that could be strengthened is",
        "A point that merits further development is",
    ]
    _RECOMMENDATION_OPENERS = [
        "It is recommended to give priority to",
        "A practical next step would be to focus on",
        "It would be beneficial to strengthen",
    ]

    def _pick_non_clashing_opener(openers: List[str], existing_options: List[str]) -> str:
        for opener in openers:
            clashes = False
            for opt in existing_options:
                if _has_same_opener(opener, opt):
                    clashes = True
                    break
            if not clashes:
                return opener
