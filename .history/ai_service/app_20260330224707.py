...
def _ensure_critical_indicator_mentioned(
    ...
    if field_name == "strengths":
        ...
        _IMPROVEMENT_TEMPLATES = [
            f"Further development is needed in {phrase}, which received a rating of {rating:.0f} and represents the most significant opportunity for instructional growth.",
            f"The area of {phrase} warrants focused professional attention, as it was rated {rating:.0f} and has clear potential for measurable improvement.",
            f"Based on the rating of {rating:.0f}, {phrase} is an area where more deliberate and consistent practice would strengthen overall instructional effectiveness.",
        ]
        replacement = _IMPROVEMENT_TEMPLATES[len(options) % len(_IMPROVEMENT_TEMPLATES)]
    else:  # recommendations
        _RECOMMENDATION_TEMPLATES = [
            f"It is recommended that the teacher prioritize {phrase}, rated at {rating:.0f}, by incorporating targeted strategies that can be consistently applied in subsequent lessons.",
            f"To address the rating of {rating:.0f} in {phrase}, it would be beneficial to implement specific, incremental adjustments focused on strengthening this aspect of instruction.",
            f"Professional development efforts should focus on {phrase}, as the rating of {rating:.0f} indicates this is a priority area where structured practice will yield the most meaningful improvement.",
        ]
...
def _build_rating_context_sentence(
    ...
    """Build a sentence summarizing the evaluator's rating for the indicator
    that best matches the feedback text. Returns empty string if no match."""
    ...
    max_scale = 4.0 if _is_peac_request(req) else 5.0
    rating_label = _score_band(rating, max_scale)
    max_int = int(max_scale)

    # Get domain average from req.averages
    sig = _evaluation_signature(req)
    domain_scores = sig.get("domains", {})
    # Find the domain average that matches this indicator's domain
    domain_avg = 0.0
    domain_avg_label = ""
    domain_lower = domain.lower()
    for d_name, d_score in domain_scores.items():
        d_lower = d_name.lower()
        # Match if domains share the same normalized name or significant overlap
        if d_lower == domain_lower or d_lower.startswith(domain_lower[:10]) or domain_lower.startswith(d_lower[:10]):
            domain_avg = d_score
            domain_avg_label = _score_band(d_score, max_scale)
            domain = d_name  # Use the canonical domain name from scores
            break

    overall = float(req.averages.overall or 0)
    overall_label = _score_band(overall, max_scale)

    # Build contextual sentence based on field type
    if field_name == "strengths":
        if domain_avg > 0:
            return (
                f"This indicator received a rating of {rating:.0f} out of {max_int} ({rating_label}), "
                f"contributing to the {domain} domain average of {domain_avg:.1f} ({domain_avg_label}) "
                f"and an overall teaching performance rating of {overall:.1f} ({overall_label})."
            )
        return (
            f"This indicator received a rating of {rating:.0f} out of {max_int} ({rating_label}), "
            f"reflecting strong performance in the overall evaluation rating of {overall:.1f} ({overall_label})."
        )
    elif field_name == "areas_for_improvement":
        if domain_avg > 0:
            return (
                f"This indicator was rated {rating:.0f} out of {max_int} ({rating_label}) "
                f"within the {domain} domain (average: {domain_avg:.1f}), "
                f"suggesting a targeted area for development relative to the overall performance of {overall:.1f} ({overall_label})."
            )
        return (
            f"This indicator was rated {rating:.0f} out of {max_int} ({rating_label}), "
            f"indicating an area for focused improvement within the overall evaluation of {overall:.1f} ({overall_label})."
        )
    else:  # recommendations
        if domain_avg > 0:
            return (
                f"Addressing this area, currently rated {rating:.0f} out of {max_int} ({rating_label}) "
                f"in the {domain} domain (average: {domain_avg:.1f}), "
                f"would contribute to raising the overall performance from {overall:.1f} ({overall_label})."
            )
        return (
            f"Addressing this area, currently rated {rating:.0f} out of {max_int} ({rating_label}), "
            f"would support improvement in the overall evaluation rating of {overall:.1f} ({overall_label})."
        )
...
def _build_domain_rating_summary(
    ...
    """Build a fallback rating context sentence using domain averages and overall
    score when no specific indicator matched the feedback text."""
    sig = _evaluation_signature(req)
    ...
    if field_name == "strengths":
        strongest = sig.get("strongest", "")
        strongest_score = domain_scores.get(strongest, 0)
        if strongest and strongest_score > 0:
            strongest_label = _score_band(strongest_score, max_scale)
            return (
                f"The strongest domain in this evaluation was {strongest} "
                f"with an average of {strongest_score:.1f} out of {max_int} ({strongest_label}), "
                f"contributing to the overall teaching performance rating of {overall:.1f} ({overall_label})."
            )
    elif field_name in ("areas_for_improvement", "recommendations"):
        weakest = sig.get("weakest", "")
        weakest_score = domain_scores.get(weakest, 0)
        if weakest and weakest_score > 0:
            weakest_label = _score_band(weakest_score, max_scale)
            if field_name == "areas_for_improvement":
                return (
                    f"The {weakest} domain received the lowest average of {weakest_score:.1f} "
                    f"out of {max_int} ({weakest_label}), highlighting this area as a priority "
                    f"for development within the overall evaluation of {overall:.1f} ({overall_label})."
                )
            else:
                return (
                    f"Strengthening practices in the {weakest} domain, which averaged "
                    f"{weakest_score:.1f} out of {max_int} ({weakest_label}), would contribute "
                    f"to improving the overall performance rating of {overall:.1f} ({overall_label})."
                )
    # Generic fallback
    if overall > 0:
        return f"The overall teaching performance was rated {overall:.1f} out of {max_int} ({overall_label})."
    return ""
...
def _enrich_with_rating_context(
    ...
    """After filler stripping, enrich the feedback text with a rating-based
    context sentence that summarizes the evaluator's actual ratings.
    Ensures the output is 2-3 sentences: substantive feedback + rating context."""
    sents = _split_sentences(text)
    # If already 3+ sentences of non-filler content, no enrichment needed
    if len(sents) >= 3:
        return text
    # Build and append rating context — try specific indicator first, then domain-level
    context_sent = _build_rating_context_sentence(text, comments, field_name, req)
    if not context_sent:
        context_sent = _build_domain_rating_summary(field_name, req)
    if context_sent:
...
def generate(req: GenerateRequest):
    ...
    strengths_options = [_enrich_with_rating_context(opt, comments, "strengths", req) for opt in strengths_options]
    improvement_options = [_enrich_with_rating_context(opt, comments, "areas_for_improvement", req) for opt in improvement_options]
    recommendation_options = [_enrich_with_rating_context(opt, comments, "recommendations", req) for opt in recommendation_options]
