...
FIELD_CLOSERS = {
    "strengths": [
        "Sustaining this practice will continue to strengthen the overall quality of instruction.",
        "Continued application of this strength will benefit both lesson delivery and student learning outcomes.",
        "This practice positively supports the learning environment and is well worth maintaining.",
    ],
    "areas_for_improvement": [
...
_OPENER_POOL_STRENGTHS = [
    ...
    "Throughout the lesson, it was apparent that",
    "One of the most effective aspects of the lesson was that",
    ...
    "What stood out most during the lesson was that",
]
    ...
    if field_name == "strengths":
        ...
        if is_top:
            pool = [
                ...
                "This practice was well-established and consistently applied, contributing to a high-quality and purposeful learning experience for the students.",
                "The strength and consistency of this practice reflected a mature level of instructional skill that positively shaped the overall lesson.",
                f"Across the lesson, this stood out as one of the most effective dimensions of the teacher's practice within {cd}.",
                "The deliberate and sustained application of this practice made it one of the most effective aspects of the lesson delivery.",
            ]
        elif is_high:
            pool = [
                ...
                "The steady application of this practice contributed to a well-paced and coherent lesson that supported learner understanding.",
                "This area of practice showed reliability and purpose, adding to the overall effectiveness of lesson delivery.",
                f"The teacher's consistent application of this skill was a notable contributor to the overall {overall_label} performance observed.",
            ]
        else:
            pool = [
                ...
                "There were visible moments of effective practice here, suggesting this is an area with strong potential for further growth.",
                "This developing competency, when reinforced through deliberate practice, can become a more consistent part of the teacher's instructional approach.",
            ]
    ...
    if field_name == "strengths":
        ...
        if strongest:
            pool = [
                ...
                f"This practice is rooted in {strongest.lower()}, which the evaluator identified as the teacher's most well-developed area of instruction.",
                "The quality and consistency of this practice reflected a well-developed instructional skill that enhanced the overall lesson experience.",
            ]
            return pool[option_idx % len(pool)]
...
def _enrich_with_rating_context(
    ...
    if context_sent:
        # Redundancy guard: check if the enrichment sentence overlaps too heavily
        # with the existing text (avoids tautology like "consistently evident...
        # Consistently strong evidence... was noted")
        text_words = set(re.sub(r"[^a-z\s]", "", text.lower()).split())
        ctx_words = set(re.sub(r"[^a-z\s]", "", context_sent.lower()).split())
        _STOP_WORDS = {
            "the", "a", "an", "of", "in", "to", "and", "is", "was", "that",
            "this", "for", "with", "as", "by", "on", "it", "from", "or", "at",
        }
        text_content = text_words - _STOP_WORDS
        ctx_content = ctx_words - _STOP_WORDS
        if text_content and ctx_content:
            overlap = len(text_content & ctx_content) / max(len(ctx_content), 1)
            if overlap > 0.45:
                # Try the next pool entry to get a less overlapping sentence
                alt_sent = _build_rating_context_sentence(text, comments, field_name, req, option_idx + 3)
                if not alt_sent:
                    alt_sent = _build_domain_rating_summary(field_name, req, option_idx + 3, feedback_text=text)
                if alt_sent:
                    alt_words = set(re.sub(r"[^a-z\s]", "", alt_sent.lower()).split()) - _STOP_WORDS
                    alt_overlap = len(text_content & alt_words) / max(len(alt_words), 1) if alt_words else 1.0
                    if alt_overlap < overlap:
                        context_sent = alt_sent
