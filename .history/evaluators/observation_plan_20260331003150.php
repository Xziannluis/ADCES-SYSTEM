...
foreach ($eval_teachers as $t) {
    ...
    // Get observers
    if ($is_leader) {
        // Leaders only see themselves as observer
        $obs_query = "SELECT DISTINCT u.name FROM evaluations e JOIN users u ON e.evaluator_id = u.id WHERE e.teacher_id = :teacher_id AND e.academic_year = :academic_year AND e.semester = :semester AND e.evaluator_id = :evaluator_id ORDER BY u.name";
        $obs_stmt = $db->prepare($obs_query);
        $obs_stmt->execute([':teacher_id' => $tid, ':academic_year' => $academic_year, ':semester' => $semester, ':evaluator_id' => $_SESSION['user_id']]);
