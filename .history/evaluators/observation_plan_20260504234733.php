...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_schedule') {
    ...
    if (!empty($teacher_id)) {
        ...
        // Find the most-recent evaluation for this teacher/semester.
        // Strategy: ALWAYS CREATE NEW unless this is an explicit reschedule.
        // Only reuse if the most recent eval is a draft with NO schedule set yet.
        // This prevents accidentally overwriting an existing scheduled evaluation
        // when the user sets a new schedule.
        $is_reschedule = !empty($_POST['is_reschedule']);
        $eval_id = null;
        
        if ($is_reschedule) {
            // For explicit reschedule: reuse the most recent evaluation (don't create new)
            ...
            $find_eval->execute([':tid' => $teacher_id, ':ay' => $lock_academic_year, ':sem' => ($post_semester ?: $lock_semester)]);
            ...
            // For initial schedule setting: only reuse if most recent eval is draft with NO schedule
            $find_eval = $db->prepare("SELECT id, status, observation_date, observation_time FROM evaluations WHERE teacher_id = :tid AND academic_year = :ay AND semester = :sem ORDER BY id DESC LIMIT 1");
            $find_eval->execute([':tid' => $teacher_id, ':ay' => $lock_academic_year, ':sem' => ($post_semester ?: $lock_semester)]);
            $eval_row = $find_eval->fetch(PDO::FETCH_ASSOC);
            if ($eval_row) {
                // Only reuse if: status is draft AND has no schedule set yet
                $has_schedule = !empty($eval_row['observation_date']) || !empty($eval_row['observation_time']);
                if ($eval_row['status'] === 'draft' && !$has_schedule) {
                    $eval_id = $eval_row['id'];
                }
                // Otherwise create new (eval_id stays null, triggering INSERT below)
            }
        }
        ...
        if ($eval_id) {
            ...
            $ins_eval->execute([
                ...
                ':ay' => $lock_academic_year,
                ':sem' => ($post_semester ?: $lock_semester),
                ':obs_date' => $observation_date,
                ':obs_time' => $observation_time,
                ':room' => $room,
                ':subject_area' => $subject_area,
...
foreach ($eval_teachers as $t) {
    ...
    if (!empty($obs_date)) {
        $norm = date('Y-m-d H:i', strtotime($obs_date));
    ...
    // President/VP names should appear only if they accepted as observer
    // (i.e., they exist in teacher_assignments), so no auto-add from scheduled_by.
