...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_schedule') {
    ...
    if (!empty($teacher_id)) {
        ...
        // Get teacher info
        $teacher_stmt = $db->prepare("SELECT faculty_name, department FROM teachers WHERE id = :id LIMIT 1");
        ...
        // Find or create evaluation record for this teacher/semester/month
        $find_eval = $db->prepare("SELECT id FROM evaluations WHERE teacher_id = :tid AND academic_year = :ay AND semester = :sem ORDER BY id DESC LIMIT 1");
        ...
        $eval_id = $eval_row ? $eval_row['id'] : null;
        ...
        $teacher_info_stmt = $db->prepare("SELECT faculty_name, department, evaluation_schedule FROM teachers WHERE id = :id LIMIT 1");
...
function openScheduleModal() {
    ...
    // Auto-select the first scheduled teacher so existing data populates
    if (select) {
        for (let i = 0; i < select.options.length; i++) {
            const opt = select.options[i];
            if (opt.dataset.hasSchedule === '1') {
                select.value = opt.value;
                // Trigger change event to populate existing data
