...
// Cancel / clear evaluation schedule (not allowed for president/vice_president)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'cancel_schedule') {
    if (in_array($_SESSION['role'], ['president', 'vice_president'])) {
        $_SESSION['error'] = 'Schedule management is handled by department Deans/Principals and Coordinators.';
        header('Location: observation_plan.php?semester=' . urlencode($_GET['semester'] ?? '1st') . '&academic_year=' . urlencode($_GET['academic_year'] ?? ''));
        exit();
    }
    $teacher_ids = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'join_observer') {
    if (in_array($_SESSION['role'], ['president', 'vice_president'])) {
        ...
        if (is_array($teacher_ids) && count($teacher_ids) > 0) {
            ...
            $_SESSION['success'] = "Joined as observer for $joined teacher(s).";
        }
    }
    $redirect = 'observation_plan.php?semester=' . urlencode($_GET['semester'] ?? '1st') . '&academic_year=' . urlencode($_GET['academic_year'] ?? '');
...
// Handle leader leave as observer
    ...
    if (in_array($_SESSION['role'], ['president', 'vice_president'])) {
        ...
        if (is_array($teacher_ids) && count($teacher_ids) > 0) {
            ...
            foreach ($teacher_ids as $tid) {
                $tid = (int)$tid;
                if ($tid <= 0) continue;
                $del = $db->prepare("DELETE FROM teacher_assignments WHERE evaluator_id = :eid AND teacher_id = :tid");
                $del->execute([':eid' => $_SESSION['user_id'], ':tid' => $tid]);
                $left += $del->rowCount();
            }
            $_SESSION['success'] = "Left as observer for $left teacher(s).";
        }
    }
    $redirect = 'observation_plan.php?semester=' . urlencode($_GET['semester'] ?? '1st') . '&academic_year=' . urlencode($_GET['academic_year'] ?? '');
    if (!empty($_GET['department'])) $redirect .= '&department=' . urlencode($_GET['department']);
...
// Handle schedule setting (not allowed for president/vice_president)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_schedule') {
    if (in_array($_SESSION['role'], ['president', 'vice_president'])) {
        $_SESSION['error'] = 'Schedule setting is managed by department Deans/Principals and Coordinators.';
        header('Location: observation_plan.php?semester=' . urlencode($_GET['semester'] ?? '1st') . '&academic_year=' . urlencode($_GET['academic_year'] ?? ''));
        exit();
    }
...
if ($is_leader) {
    // President/VP: show only evaluations they personally made
...
if ($is_leader) {
    // President/VP: show only teachers they personally scheduled
    $sched_query = "SELECT DISTINCT t.id, t.name, t.department as teacher_department,
                    ...
                    WHERE t.status = 'active'
                      AND t.scheduled_by = :scheduled_by
    ...
    $sched_stmt->bindParam(':scheduled_by', $_SESSION['user_id']);
            ...
            <div style="background: white; padding: 30px;">
                ...
                <div class="mb-3 d-flex justify-content-end align-items-center gap-2 no-print">
                    ...
                    <button class="btn btn-success" id="joinObserverBtn" disabled onclick="joinAsObserver()">
                        <i class="fas fa-user-plus me-1"></i>Join as Observer
                    </button>
                    <button class="btn btn-outline-danger" id="leaveObserverBtn" disabled onclick="leaveAsObserver()">
                        <i class="fas fa-user-minus me-1"></i>Leave as Observer
                    </button>
                    <?php endif; ?>
                </div>
                                ...
                                <tr>
                                    <td>
                                        <?php if ($is_leader): ?>
                                            ...
                                            <input type="checkbox" class="form-check-input observer-opt-check no-print" value="<?php echo (int)$tid; ?>" <?php echo $is_opted ? 'checked' : ''; ?> data-opted="<?php echo $is_opted ? '1' : '0'; ?>" style="width:16px;height:16px;cursor:pointer;margin-right:6px;vertical-align:middle;" title="<?php echo $is_opted ? 'You are an observer' : 'Check to join as observer'; ?>">
                                        <?php elseif ($has_schedule && !$is_done): ?>
...
function joinAsObserver() {
    ...
    if (!confirm('Join as observer/evaluator for ' + ids.length + ' teacher(s)?')) return;
    var form = document.createElement('form');
    form.method = 'POST';
...
function leaveAsObserver() {
    ...
    if (ids.length === 0) return;
    if (!confirm('Leave as observer for ' + ids.length + ' teacher(s)?')) return;
    var form = document.createElement('form');
    form.method = 'POST';
    form.action = window.location.href;
