...
// When a dean/principal views a coordinator's teachers, restrict to the dean's own department
// so that e.g. a CCIS dean only sees CCIS teachers, not CAS teachers
if ($viewing_coordinator && in_array($_SESSION['role'], ['dean', 'principal'])) {
    $dean_department = trim($_SESSION['department'] ?? '');
    if ($dean_department !== '') {
        $target_programs = array_values(array_filter($target_programs, function ($prog) use ($dean_department) {
            return strcasecmp($prog, $dean_department) === 0;
        }));
        // If intersection is empty, fall back to dean's own department
        if (empty($target_programs)) {
            $target_programs = [$dean_department];
        }
    }
}
