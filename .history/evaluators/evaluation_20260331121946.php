...
<body>
            ...
            <div class="card mb-4" id="teacherSelection">
                ...
                <div class="card-body">
                    ...
                    <div class="list-group" id="teacherList">
                        ...
                        ?>
                        <div class="list-group-item teacher-item <?php echo ($can_evaluate_now && !$all_done) ? '' : 'disabled'; ?>" data-teacher-id="<?php echo $teacher_row['id']; ?>" data-teacher-name="<?php echo htmlspecialchars($teacher_row['name'] ?? '', ENT_QUOTES); ?>" data-has-schedule="<?php echo $has_schedule ? '1' : '0'; ?>" data-can-evaluate-now="<?php echo ($can_evaluate_now && !$all_done) ? '1' : '0'; ?>" data-schedule-message="<?php echo htmlspecialchars($schedule_message, ENT_QUOTES); ?>" data-block-reason="<?php echo htmlspecialchars($all_done ? 'You have already completed the evaluation for this teacher.' : $schedule_block_message, ENT_QUOTES); ?>" data-focus="<?php echo htmlspecialchars($teacher_row['evaluation_focus'] ?? '', ENT_QUOTES); ?>" data-semester="<?php echo htmlspecialchars($teacher_row['evaluation_semester'] ?? '', ENT_QUOTES); ?>" data-subject-area="<?php echo htmlspecialchars($teacher_row['evaluation_subject_area'] ?? '', ENT_QUOTES); ?>" data-room="<?php echo htmlspecialchars($teacher_row['evaluation_room'] ?? '', ENT_QUOTES); ?>" data-subject="<?php echo htmlspecialchars($teacher_row['evaluation_subject'] ?? '', ENT_QUOTES); ?>" data-form-type="<?php echo htmlspecialchars($teacher_form_type, ENT_QUOTES); ?>" data-iso-done="<?php echo $iso_done ? '1' : '0'; ?>" data-schedule-raw="<?php echo htmlspecialchars($teacher_row['evaluation_schedule'] ?? '', ENT_QUOTES); ?>" data-teacher-department="<?php echo htmlspecialchars($teacher_row['department'] ?? '', ENT_QUOTES); ?>" data-scheduled-department="<?php echo htmlspecialchars($teacher_row['scheduled_department'] ?? '', ENT_QUOTES); ?>">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <p class="mb-0 text-muted"><?php
                                        ...
                                        // Leaders see the teacher's own department
                                        $eval_dept = $_SESSION['department'] ?? '';
                                        $display_dept = $teacher_row['department'];
                                        if (!$is_leader && $teacher_row['department'] !== $eval_dept && !empty($eval_dept)) {
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    <?php if ($all_done): ?>
                                        <span class="badge bg-info p-2"><i class="fas fa-check-circle me-1"></i>Already evaluated</span>
                                        <i class="fas fa-chevron-right ms-2"></i>
                                    <?php elseif ($teacher_form_type === 'both' && $can_evaluate_now): ?>
                                        <?php if ($iso_done): ?>
    ...
    if (textarea) textarea.value = text || textarea.value;
        ...
        function showAISuggestions(data = {}, dbg = {}) {
            ...
            // Auto-fill textareas with the primary AI suggestion so comments are never left blank
            const strengthsTA = document.getElementById('strengths');
            const improvementTA = document.getElementById('improvementAreas');
            const recommendationsTA = document.getElementById('recommendations');
            if (strengthsTA && data.strengths && (!strengthsTA.value.trim())) {
                strengthsTA.value = data.strengths;
            }
            if (improvementTA && data.improvement_areas && (!improvementTA.value.trim())) {
                improvementTA.value = data.improvement_areas;
            }
            if (recommendationsTA && data.recommendations && (!recommendationsTA.value.trim())) {
                recommendationsTA.value = data.recommendations;
            }
