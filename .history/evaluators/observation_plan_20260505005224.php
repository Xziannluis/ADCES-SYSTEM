...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_schedule') {
    ...
    if (!empty($teacher_id)) {
        ...
        // Determine if this is a reschedule operation. Treat as reschedule
        // only when both the reschedule flag and the reschedule_teacher_id
        // are provided from the modal. This avoids accidentally updating
        // an existing evaluation when the user intended to create a new one.
        $is_reschedule = (!empty($_POST['is_reschedule']) && !empty($_POST['reschedule_teacher_id']));
        $eval_id = null;
        ...
        if ($is_reschedule) {
            ...
            $find_eval->execute([':tid' => $teacher_id, ':ay' => $academic_year, ':sem' => ($post_semester ?: $semester)]);
