...
foreach ($eval_teachers as $t) {
    ...
    if ($is_leader && !empty($raw_department) && $is_secondary_dept) {
        // Leader filtering by a specific department for a cross-department teacher: only show evaluators from that department
    ...
    if ($is_leader) {
        ...
        // If filtering by a specific department and teacher's primary dept differs, use filtered dept
        $leader_dean_dept = $teacher_dept;
        if (!empty($raw_department) && $raw_department !== $teacher_dept) {
            $leader_dean_dept = $raw_department;
        }
        if (!empty($leader_dean_dept)) {
            ...
            $dept_dean_stmt->execute([':dept' => $leader_dean_dept]);
...
foreach ($scheduled_teachers as $t) {
    ...
    // For leaders: always add dean/principal of the relevant department; coordinators only if assigned
    if ($is_leader) {
        ...
        // If filtering by a specific department and it differs, use filtered dept
        if (!empty($raw_department) && $raw_department !== $dept_to_show) {
            $dept_to_show = $raw_department;
        }
...
if (!empty($filter_status)) {
    $teachers_list = array_filter($teachers_list, function($t) use ($eval_data, $filter_status) {
        $is_done = $eval_data[$t['id']]['done'] ?? false;
