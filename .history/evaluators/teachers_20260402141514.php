...
if ($_POST && isset($_POST['action']) && $_POST['action'] === 'update_schedule') {
    ...
    if (!empty($teacher_id)) {
        ...
        if ($stmt->execute()) {
            ...
            notifyScheduleParticipants($db, $teacher_id, $schedule, $room, $_SESSION['user_id'], $_SESSION['name'] ?? 'Evaluator', $_SESSION['role'] ?? '', $sched_dept);
        } else {
...
if ($is_leader) {
    // Leaders see ALL active teachers across all departments
    $stmt = $db->prepare("SELECT t.* FROM teachers t WHERE t.status = 'active' ORDER BY t.department, t.name ASC");
    $stmt->execute();
    $teachers = $stmt;
