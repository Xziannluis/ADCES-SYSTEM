...
foreach ($eval_teachers as $t) {
    ...
    if (!empty($obs_date)) {
        ...
        if ($obs_time_fmt !== '' && $obs_time_fmt !== '00:00:00' && $obs_time_fmt !== '00:00') {
            ...
            $end_fmt = '';
            // Check if observation date matches schedule end date to get end time
            if (!empty($sched_dt_end)) {
                $obs_date_key = date('Y-m-d', strtotime($obs_date));
                $sched_end_date_key = date('Y-m-d', strtotime($sched_dt_end));
                if ($obs_date_key === $sched_end_date_key) {
