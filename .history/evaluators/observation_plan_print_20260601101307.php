...
if ($is_leader) {
    ...
    $query = "SELECT DISTINCT t.id, t.name, t.department as teacher_department,
                     ...
                     e.id as eval_id, e.observation_date, e.observation_time, e.observation_time_end, e.status as eval_status, e.faculty_signature,
...
foreach ($eval_teachers as $t) {
    ...
    if (!empty($obs_date)) {
        ...
        $obs_time_end_fmt = trim((string)($t['observation_time_end'] ?? ''));
        ...
        if ($obs_time_fmt !== '' && $obs_time_fmt !== '00:00:00' && $obs_time_fmt !== '00:00') {
            ...
            $end_fmt = '';
            // First check for explicit observation_time_end field
            if ($obs_time_end_fmt !== '' && $obs_time_end_fmt !== '00:00:00' && $obs_time_end_fmt !== '00:00') {
                $end_fmt = date('g:i A', strtotime($obs_time_end_fmt));
            } else {
                // Fallback to schedule end time
