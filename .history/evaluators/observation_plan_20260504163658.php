...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_schedule') {
    ...
    if (!empty($teacher_id)) {
        ...
        // Do NOT reuse a record that is already completed, and prefer
        // creating a new evaluation when the scheduled date/time differs
        // from the existing record. This prevents later schedule sets
        // from overwriting an already-completed evaluation.
        ...
        if ($eval_row) {
            $reuse = true;
            // Don't reuse a completed evaluation
            if (isset($eval_row['status']) && $eval_row['status'] === 'completed') {
                $reuse = false;
            }
            // If we have a schedule date/time, only reuse when it matches exactly
            if ($reuse && !empty($observation_date) && !empty($observation_time)) {
                $existing_date = trim((string)($eval_row['observation_date'] ?? ''));
                $existing_time = trim((string)($eval_row['observation_time'] ?? ''));
                if ($existing_date !== $observation_date || $existing_time !== $observation_time) {
                    $reuse = false;
                }
            }
            if ($reuse) {
                ...
                $eval_id = null; // force create new evaluation record
            }
        }
