...
$report_type = ($_GET['report_type'] ?? 'single') === 'single' ? 'single' : 'report';
$academic_year = preg_match('/^\d{4}-\d{4}$/', $_GET['academic_year'] ?? '') ? $_GET['academic_year'] : '2023-2024';
$semester = preg_match('/^(1st|2nd|summer)?$/', $_GET['semester'] ?? '') ? $_GET['semester'] : '';
$teacher_id = (int)($_GET['teacher_id'] ?? 0);

// Verify the user has access to the requested evaluation
if ($evaluation_id > 0) {
    $accessCheck = $db->prepare(
        "SELECT e.id FROM evaluations e
         LEFT JOIN teachers t ON e.teacher_id = t.id
         WHERE e.id = :eval_id
           AND (e.evaluator_id = :user_id OR t.department = :dept)
         LIMIT 1"
    );
    $accessCheck->execute([
    ...
    if (!$accessCheck->fetch()) {
        ...
        die('Access denied: you do not have permission to export this evaluation.');
    }
}
...
function exportEvaluationForm($evaluation_id, $academic_year, $semester) {
    ...
    <head>
        ...
        <title>Classroom Evaluation Form</title>
