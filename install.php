...
// Ensure photo BLOB columns exist (for portable SQL exports)
$photo_alters = [
    "ALTER TABLE `teachers` ADD COLUMN `photo_data` LONGBLOB DEFAULT NULL AFTER `photo_path`",
    "ALTER TABLE `teachers` ADD COLUMN `photo_mime` VARCHAR(50) DEFAULT NULL AFTER `photo_data`",
    "ALTER TABLE `users` ADD COLUMN `photo_data` LONGBLOB DEFAULT NULL AFTER `photo_path`",
    "ALTER TABLE `users` ADD COLUMN `photo_mime` VARCHAR(50) DEFAULT NULL AFTER `photo_data`",
];
foreach ($photo_alters as $alter) {
    try {
        $db->exec($alter);
    } catch (PDOException $e) {
...
echo "<p>✓ Ensured photo BLOB columns exist in teachers and users tables</p>";
