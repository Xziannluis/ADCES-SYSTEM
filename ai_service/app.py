...
def _ensure_critical_indicator_mentioned(
    ...
    if field_name == "strengths":
        opener = _pick_non_clashing_opener(_STRENGTH_OPENERS, options)
        replacement = (
            f"{opener} {phrase}. "
        ...
        opener = _pick_non_clashing_opener(_IMPROVEMENT_OPENERS, options)
        replacement = (
            f"{opener} {phrase}. "
        ...
        opener = _pick_non_clashing_opener(_RECOMMENDATION_OPENERS, options)
        replacement = (
            f"{opener} {phrase}, "
...
def _natural_criterion_reference(text: str) -> str:
    ...
    cleaned = re.sub(r"^[Ff]acilitate\s+", "facilitating ", cleaned)
...
def _paraphrase_dataset_feedback(req: GenerateRequest, field_name: str, retrieved: List[Dict[str, Any]], fallback_query: str) -> str:
    ...
    return _trim_to_sentences(selected, 3)
...
_BANNED_PHRASE_PATTERNS = [
    ...
    (re.compile(r"\bob(?:servation|served)\b", re.IGNORECASE), "the lesson"),
...
def _make_three_options(base_text: str, req: GenerateRequest, field_name: str, retrieved: List[Dict[str, Any]]) -> List[str]:
    ...
    # Collect unique, trimmed feedback texts
    candidates: List[str] = []
    seen_fp: set = set()
    for item in retrieved:
        text = _normalize_sentence(item.get("feedback_text") or item.get("text") or "")
        ...
        text = _trim_to_sentences(text, 3)
    ...
    # Pick 3 distinct options, skipping near-duplicates via word overlap AND opener checks
    out: List[str] = []
