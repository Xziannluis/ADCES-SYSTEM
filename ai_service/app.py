...
def _vary_sentence_starters(sentences: List[str]) -> List[str]:
    ...
    # Detect two-word prefix duplicates (e.g. "During the", "The teacher")
    def _two_word_key(s: str) -> str:
        words = s.split()
    ...
    _PREFIX_ALTS: Dict[str, List[str]] = {
        "during the": ["Throughout the", "In the course of the", "Over the"],
        "the teacher": ["The educator", "The instructor", "Notably, the teacher"],
        "the lesson": ["The class session", "The instructional period", "Notably, the lesson"],
        "as noted": ["As reflected", "It was clear", "Clearly"],
        "based on": ["Drawing from", "According to", "As supported by"],
        "it was": ["Additionally, it was", "Furthermore, it was", "Moreover, it was"],
        "this was": ["This became", "In particular, this was", "Notably, this was"],
        "a clear": ["A distinct", "An evident", "A noteworthy"],
        "one notable": ["A key", "An important", "A significant"],
    }
    ...
    for s in sentences:
        key = _two_word_key(s)
        if prefix_counts.get(key, 0) > 1 and key in _PREFIX_ALTS and len(result) > 0:
            alt_idx = used_alts.get(key, 0)
            alts = _PREFIX_ALTS[key]
            if alt_idx < len(alts):
                words = s.split()
                rest = " ".join(words[2:])
                if rest:
                    rewritten = f"{alts[alt_idx]} {rest[0].lower() + rest[1:]}"
                else:
                    rewritten = alts[alt_idx]
                result.append(_normalize_sentence(rewritten))
                used_alts[key] = alt_idx + 1
            else:
                result.append(s)
        else:
            result.append(s)
